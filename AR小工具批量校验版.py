...
#批量返回遍历回归结果和前瞻倍数结果
def auto_cal(start_date,end_date,y,factor_number,x_real,y_real,wgt,x_base_pred,pred_date):
    # 遍历回归
    ...
    #将结果储存到字典
    auto_result = {'预测日期':pred_date,'行业':y,'前瞻倍数':multiples}
    return auto_result

def make_map_to_date_frame(map, valueC):
    ...
    datas = {}
    datas[valueC] = values
    dataPd = pd.DataFrame.from_dict(datas)
    dataPd.index = keys
    return dataPd


def make_date_frame(x_list, x_field_list):
    datas = []
    data_list = make_data_list(x_list, x_field_list,datas)
    test_data = pd.DataFrame.from_dict(data_list)
    test_data.index = datas
    return test_data


def make_data_list(x_list, x_field_list,datas):
    data_list = []
    for index in range(len(x_list)):
        data_item = {}
        x_item = x_list[index]
        datas.append(x_item['预测日期'])
        for x_field in x_field_list:
            data_item[x_field] = x_item.get(x_field, None)
        data_list.append(data_item)
    return data_list

# 使用须知：请将<AR小工具>文件夹拷贝至C盘根目录
...
factor_number = 3
# 入参3：行业列表
y_list = list(y_real.columns)
#入参4：前瞻日期
pred_date_list = ['2020/12/31','2021/12/31','2022/12/31']
...
for pred_date in pred_date_list:
    for y in y_list:
        auto_result = auto_cal(start_date,end_date,y,factor_number,x_real,y_real,wgt,x_base_pred,pred_date)
        auto_result_list.append(auto_result)

auto_result_df = make_date_frame(auto_result_list, ['预测日期','行业','前瞻倍数'])
auto_result_df.to_excel('/Users/andy/Desktop/AR小工具/Result/前瞻倍数批量计算结果.xlsx')
